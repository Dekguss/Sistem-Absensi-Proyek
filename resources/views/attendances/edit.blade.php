        ...
        <form action="{{ route('projects.attendances.update', [$project, $attendance]) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="worker_id" name="worker_id" required>
                    ...
                    <option value="{{ $worker->id }}" {{ $worker->id == $attendance->worker_id ? 'selected' : '' }}>{{ $worker->name }} ({{ ucfirst($worker->role) }})</option> 
                    @endforeach
                </select>
            </div>
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="time" class="form-control" id="check_in" name="check_in" value="{{ $attendance->check_in }}" required>
                </div>
                <div class="col-md-6">
                    <label for="check_out" class="form-label">Check Out</label>
                    <input type="time" class="form-control" id="check_out" name="check_out" value="{{ $attendance->check_out }}" required>
                </div>
            </div>
            <div class="mb-3">
                <label for="notes" class="form-label">Catatan</label>
                <textarea class="form-control" id="notes" name="notes" rows="2">{{ $attendance->notes }}</textarea>
            </div>
